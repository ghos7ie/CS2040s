 ...
 *
 * CASE 2.1: FEDERER LOST A SET ALR, SO MATCH IS VOID 
 * CASE 2.1: FEDERER LOSE SET
 * CASE 2.2: FEDERER LOSE SET
 *
 * CASE 3.1: EITHER HAS 2 WINS BUT STILL CONTINUE CASE 
 * 3.2: NOT ENOUGH WINS ON BOTH SIDES
 *
 * FOR FIRST 2 SETS 
 * CASE 4.1: EITHER IS MORE THAN 8 WINS CASE 
 * 4.2: IF BOTH SIDE >= 6, BUT DIFF IS NOT 1 CASE 
 * 4.3: IF EITHER IS MORE THAN 7, BUT DIFF IS 3 OR MORE CASE 
 * 4.4: IF EITHER IS 6, BUT OTHER IS NOT 5
 *
 * FOR LAST SET 
 * CASE 5.1: DIFF IS NOT 2 WHEN SCORE FOR BOTH IS 6 OR MORE 
 * CASE 5.2: WHEN EITHER IS LESS THAN 6, BUT DIFF IS
 *
 * CASE 6: TIE
 *
      ...
      setLoop:
        for (int j = 0; j < noOfSets; j++) {
          ...
          if (j <= 1) {
            ...
            if (p1Score >= 6 && p2Score >= 6
                && Math.abs(p1Score - p2Score) != 1) {
              valid = false;
              break setLoop;
            }
            // CASE 4.3: if either is 7 or more, but diff is more than 2
            if ((p1Score >= 7 || p2Score >= 7)
                && Math.abs(p1Score - p2Score) >= 3) {
              valid = false;
              break setLoop;
            }
            // CASE 4.4: either is 6 but the other is not 5
