...
CASE 3.1: EITHER HAS 2 WINS BUT STILL CONTINUE
CASE 3.2: NOT ENOUGH WINS ON BOTH SIDES

FOR FIRST 2 SETS
CASE 4.1: EITHER IS MORE THAN 8 WINS
CASE 4.2: IF BOTH SIDE >= 6, BUT DIFF IS NOT 1
CASE 4.3: IF EITHER IS MORE THAN 7, BUT DIFF IS 3 OR MORE
        ...
        if (noOfSets != 2 && noOfSets != 3) {
          ...
          setLoop: for (int j = 0; j < noOfSets; j++) {
            ...
            if (j >= 2) {
              ...
              if (p1Score >= 6 && p2Score >= 6 && (Math.abs(p1Score - p2Score) >= 3 || Math.abs(p1Score - p2Score) == 1)) {
                valid = false;
                break setLoop;
              }
